...
public class FlockersWithUI extends GUIState {
    ...
    public static void main(String[] args) {
        new FlockersWithUI(args).createController();  // randomizes by currentTimeMillis
    }
    ...
    public FlockersWithUI(String[] args) {
        super(new Flockers(System.currentTimeMillis(), args));
    }
    ...
    public void setupPortrayals() {
        ...
        for(int x = 0; x < flock.flockers.allObjects.numObjs; x++) {
            boolean adhocPresent = (flock.numAdhoc > 0);

            Color adhocColor = new Color(250, 128, 128);
            Color normalBoidColor = adhocPresent ? 
                new Color(255, 255, 255) :
                new Color(128 + guirandom.nextInt(128), //removing many colors
                       128 + guirandom.nextInt(128),
                       128 + guirandom.nextInt(128));

            Color chosenColor;
            if (((Flocker)flock.flockers.allObjects.objs[x]).adhoc) {
                chosenColor = adhocColor;
            } else {
                chosenColor = normalBoidColor;
            }
